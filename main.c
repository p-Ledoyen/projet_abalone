...
int main()
{
	...
	screen = SDL_SetVideoMode(h, w, 32, SDL_HWSURFACE|SDL_DOUBLEBUF);
	
	SDL_Rect backgroundPosition={0,0};
	SDL_Surface* background=IMG_Load("grille_abalone_1181.png");
	SDL_BlitSurface(background, NULL, screen, &backgroundPosition);
	display_grid(T_reference);
	COORD P1, P2, P3, P4;
	...
	while(!in.key[SDLK_ESCAPE] && !in.quit)
	{
		...
		if(in.mousebuttons[SDL_BUTTON_LEFT])
		{
			COORD caterpillar[5];
			int size=1;
			MOVEMENT caterpillarDirection=no;
			
			caterpillar[0]=square_calculation(in, T_reference);
			
			while(in.mousebuttons[SDL_BUTTON_LEFT])
			{
				UpdateEvents(&in);
				caterpillar[size]=square_calculation(in, T_reference);
				caterpillarDirection=square_direction(caterpillar, 0, size);
				
				printf("%i\n",size);
				if(T_reference[caterpillar[size].x][caterpillar[size].y]==T_reference[caterpillar[0].x][caterpillar[0].y] && caterpillarDirection!=no)
				{
					size++;
				}
				else if(size>1 && caterpillar[size].x==caterpillar[size-1].x && caterpillar[size].y==caterpillar[size-1].y )
				{
					size--;
				}
				else if(T_reference[caterpillar[size].x][caterpillar[size].y]!=T_reference[caterpillar[0].x][caterpillar[0].y])
				{
					MOVEMENT moveDirection=square_direction(caterpillar, size-1, size);
					POSSIBLE align;
					switch(size)
					{
						case 1:
							align=move_possible_1(caterpillar[0],moveDirection, T_daisy, player);
							if(align!=impossible)
							{
								display_move_possible_1(caterpillar[0], T_daisy, moveDirection);
								UpdateEvents(&in);
								if(!in.mousebuttons[SDL_BUTTON_LEFT])
								{
									move_1(caterpillar[0],moveDirection,align,T_daisy);
								}
							}
							break;
						case 2:
							align=move_possible_2(caterpillar[0],caterpillar[1],moveDirection, T_reference, player);
							if(align!=impossible)
							{
								display_move_possible_2(caterpillar[0],caterpillar[1], T_reference, moveDirection);
								UpdateEvents(&in);
								if(!in.mousebuttons[SDL_BUTTON_LEFT])
								{
									move_2(caterpillar[0],caterpillar[1],moveDirection,align,T_reference);
								}
							}
							break;
						case 3:
							align=move_possible_3(caterpillar[0],caterpillar[1],caterpillar[2],moveDirection, T_reference, player);
							if(align!=impossible)
							{
								display_move_possible_3(caterpillar[0],caterpillar[1],caterpillar[2], T_reference, moveDirection);
								UpdateEvents(&in);
								if(!in.mousebuttons[SDL_BUTTON_LEFT])
								{
									move_3(caterpillar[0],caterpillar[1],caterpillar[2],moveDirection,align,T_reference);
								}
							}
							break;
						default:
...
int main ()
{
    ...
    SDL_WM_SetIcon(IMG_Load("v.bmp"), NULL);
 
    ecran = SDL_SetVideoMode(1181, 1181, 32, SDL_HWSURFACE);
    SDL_WM_SetCaption("Abalone", NULL);
	
	
    imageDeFond = IMG_Load("grille_abalone_1181.png");
    //imageDeFond=shrinkSurface(imageDeFond, 2, 2);
    imageDeFond=rotozoomSurface(imageDeFond, 0,2,1);
...
void display_grid()
{
	...
	SDL_Surface *imagePionP1 = NULL, *imagePionP2 = NULL;
	imagePionP1=IMG_Load("v.png");
	imagePionP2=IMG_Load("b.png");
	for(int i=1; i<10; i++)
	{
		positionPion.y=height_margin+(i-1)*59;
		for(int j=2; j<19; j++)
		{
			
			if(j%2==0)
			{
				positionPion.x=width_margin_even+71*(j-2)/2;
			}
			else
			{
				positionPion.x=width_margin_odd+71*(j-3)/2;
			}
			if(T_reference[i][j]==1)
			{
				printf("J1\n");
				SDL_BlitSurface(imagePionP1, NULL, ecran, &positionPion);
			}
			else if (T_reference[i][j]==2)
			{
