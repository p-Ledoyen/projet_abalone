...
int Min(CASE T_bis[11][21], int depth, int alpha, int beta, PLAYER player_bis)
{
	...
	if(depth==0) //ou la partie se termine, condition à rajouter.
	...
	while(courant!=NULL)
	{
		for(i=0;i<6;i++)
		{
			...
			if(result != impossible) move( courant->p1,courant->p2, courant->p3, dep, result,T_bis);	
			Min_val=minimum(Min_val,Max(T_bis, depth-1, alpha, beta, opponent));
			if(Min_val<=alpha) return Min_val;
			beta=minimum(beta,Min_val);
			copy_tab(T_tmp,T_bis); //Annulation du coup effectué.
		}
		courant=courant->suivant;
	}
	return Min_val;
...
MOVEMENT alpha_beta(CASE T_bis[11][21], int depth, int alpha, int beta, PLAYER player_bis)
{
	int tmp, nb, i;
	int Max_val=-10000;
	POSSIBLE result;
